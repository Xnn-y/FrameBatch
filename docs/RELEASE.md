...
`tools/ffmpeg` 是可选目录。若打包前仓库中存在 `tools/ffmpeg/ffmpeg.exe` 和 `tools/ffmpeg/ffprobe.exe`，PyInstaller 会自动把它们复制到发布目录的 `_internal/tools/ffmpeg/`。应用同时兼容用户手动放在 `FrameBatch.exe` 同级的 `tools/ffmpeg/`。
...
2. 不放置 FFmpeg 时启动，确认界面显示 FFmpeg 不可用，并能手动选择 `ffmpeg.exe`。
...
- 用户配置和历史记录默认写入 `%APPDATA%/FrameBatch/`。
- 如果发布包内置 FFmpeg，请同时检查并随包提供所选 FFmpeg 构建对应的许可证说明。
- 第一版发布包可以不内置 FFmpeg，让用户在界面中配置本机 `ffmpeg.exe`。
